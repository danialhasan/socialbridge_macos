...
const createWindow = (): void => {
  ...
  mainWindow.webContents.on('did-finish-load', () => {
    ...
    const [
      stringifiedInjectionFn,
      stringifiedButtonFunctionalityFn,
      stringifiedSetMenuHTML,
    ] = stringifyFunctions([
      injectButton,
      injectButtonFunctionality,
      setMenuHTML,
    ]);
    const executeScript0 = '"' + stringifiedSetMenuHTML + ' setMenuHTML()"';
    const executeScript1 = `
    const script = document.createElement("script")
    script.id = "SocialBridge_Script"
    document.head.appendChild(script)
    `;
    const executeScript2 = `document.getElementById('SocialBridge_Script').innerHTML = "${stringifiedInjectionFn} ${stringifiedButtonFunctionalityFn} injectButton(); injectButtonFunctionality(); "`;
    console.log('<-------------->');
